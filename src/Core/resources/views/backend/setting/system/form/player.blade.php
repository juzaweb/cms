<form method="post" action="{{ route('admin.setting.save') }}" class="form-ajax">
    ...
    <div class="form-group">
        <label class="col-form-label" for="player_watermark">@lang('mymo_core::app.player_watermark')</label>
        <select name="player_watermark" id="player_watermark" class="form-control">
            <option value="1" @if($player_watermark == 1) selected @endif>@lang('mymo_core::app.enabled')</option>
            <option value="0" @if($player_watermark == 0) selected @endif>@lang('mymo_core::app.disabled')</option>
        </select>
    </div>

    <div class="form-group">
        <label class="col-form-label" for="logo">@lang('mymo_core::app.player_watermark_logo') <span class="float-right"><a href="javascript:void(0)" data-input="player_watermark_logo" data-preview="preview-watermark_logo" class="lfm"><i class="fa fa-edit"></i> @lang('mymo_core::app.change_image')</a></span></label>
