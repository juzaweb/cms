...
<div class="form-product-list">
    ...
    <div class="theme-setting theme-setting--text editor-item">
        ...
        <select name="{{ $card['code'] }}[{{ $input['name'] }}][ctype]" class="next-input select-ctype">
            <option value="1" @if($ctype == 1) selected @endif>{{ trans('mymo_core::app.genre') }}</option>
            <option value="2" @if($ctype == 2) selected @endif>{{ trans('mymo_core::app.type') }}</option>
            <option value="3" @if($ctype == 3) selected @endif>{{ trans('mymo_core::app.country') }}</option>
        </select>
    </div>

    <div class="theme-setting theme-setting--text editor-item ctype ctype-1 @if($ctype != 1) box-hidden @endif">
        <label class="next-label">{{ trans('mymo_core::app.genre') }}</label>
    ...
    <div class="theme-setting theme-setting--text editor-item ctype ctype-3 @if($ctype != 3) box-hidden @endif">
        <label class="next-label">{{ trans('mymo_core::app.country') }}</label>
    ...
    <div class="theme-setting theme-setting--text editor-item">
        <label class="next-label">{{ trans('mymo_core::app.format') }}</label>
        <select name="{{ $card['code'] }}[{{ $input['name'] }}][format]" class="next-input">
            <option value="">--- {{ trans('mymo_core::app.format') }} ---</option>
            <option value="1" @if($format == 1) selected @endif>{{ trans('mymo_core::app.movies') }}</option>
            <option value="2" @if($format == 2) selected @endif>{{ trans('mymo_core::app.tv_series') }}</option>
        </select>
    </div>

    <div class="theme-setting theme-setting--text editor-item">
        <label class="next-label">{{ trans('mymo_core::app.sort') }}</label>
        <select name="{{ $card['code'] }}[{{ $input['name'] }}][order]">
            <option value="updated_at_DESC" @if($order == 'updated_at_DESC') selected @endif>{{ trans('mymo_core::app.latest') }}</option>
            <option value="updated_at_ASC" @if($order == 'updated_at_ASC') selected @endif>{{ trans('mymo_core::app.oldest') }}</option>
            <option value="view_DESC" @if($order == 'view_DESC') selected @endif>{{ trans('mymo_core::app.views_high_to_low') }}</option>
            <option value="view_ASC" @if($order == 'view_ASC') selected @endif>{{ trans('mymo_core::app.views_low_to_high') }}</option>
        </select>
    </div>

    <div class="theme-setting theme-setting--text editor-item">
        <label class="next-label">{{ trans('mymo_core::app.limit') }}</label>
