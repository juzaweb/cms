<li class="nav-item @if(!empty($children)) dropdown has-submenu menu-large hidden-md-down hidden-sm-down hidden-xs-down @endif">
    <a class="nav-link @if(!empty($children)) dropdown-toggle @endif" href="{{ $item->link }}" @if(!empty($children)) id="dropdown-{{ $item->id }}" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false" @endif>{{ $item->label }}</a>
